...
const TestPage: React.FC<ITestPageProps> = ({ selectedSkillIds, skillsetOptions, welcomeName, userEmail, onLogout, onBack }) => {
  ...
  const [currentDraftId, setCurrentDraftId] = useState<number | null>(null); // ✅ track current draft
  ...
  const fetchQuestions = async (skillId: number) => {
    try {
      ...
      setCurrentDraftId(null); // new test session
    } catch (error) {
...
const handleSaveDraft = async () => {
  ...
  const handleLoadDraft = (draft: ISavedTest) => {
    ...
    setSubmitted(false);
    setShowResults(false);
    setCurrentDraftId(draft.Id); // ✅ remember this draft is being edited
  };
...
return (
  <div style={{ padding: 20 }}>
    ...
    <div style={{
      background: 'linear-gradient(to right, #0078d4, #2899f5)',
      color: 'white',
      padding: '20px',
      borderRadius: '8px',
      marginBottom: '20px',
      ...
      <Text variant="xLarge">Welcome to your Dashboard, {welcomeName}</Text>
      <DefaultButton text="Logout" onClick={onLogout} />
    </div>

    {/* White Card Wrapper for Skillset Test section */}
    <div style={{
      background: 'white',
      padding: '20px',
      borderRadius: '8px',
              ...
              <tbody>
                {previousResults.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ textAlign: 'center', padding: '8px 12px' }}>{item["Passed_x003f_"] === "Yes" ? "Passed" : "Failed"}</td>
                    <td style={{ textAlign: 'right', padding: '8px 12px' }}>{new Date(item.DateTaken).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {selectedSkillId && questions.length > 0 && (
        <div style={{ marginTop: 30 }}>
          <Text variant="xLarge">Skill Test: {skillsetOptions.find(opt => opt.key === selectedSkillId)?.text}</Text>
            ...
            <div key={q.Id} style={{ marginBottom: 25, padding: 10, border: '1px solid #ccc', borderRadius: 8 }}>
              ...
              <Stack tokens={{ childrenGap: 8 }} style={{ marginTop: 10 }}>
                {(['OptionA', 'OptionB', 'OptionC', 'OptionD'] as const).map(optionKey => {
                  ...
                  return (
                    <label key={optionKey} style={{ display: 'block', cursor: submitted ? 'default' : 'pointer', marginBottom: 6, backgroundColor, padding: 6, borderRadius: 4 }}>
                      <input
                        type="radio"
                        name={`question-${q.Id}`}
                        value={keyLetter}
                      ...
                      />
                      {optionValue}
                      {submitted && isSelected && <strong style={{ marginLeft: 10 }}>{isCorrect ? '✅ Correct' : '❌ Your Answer'}</strong>}
                      {submitted && !isSelected && isCorrect && <strong style={{ marginLeft: 10, color: 'green' }}>✔ Correct Answer</strong>}
                    </label>
                  );
                })}
              </Stack>
  ...
  <MessageBar
    ...
    styles={{
      ...
      root: {
        ...
        fontWeight: 500,
        alignItems: 'center',
        paddingLeft: 12 // ✅ remove icon space
      },
      content: { display: 'flex', alignItems: 'center' } // ✅ center content
    }}
  >
