...
if (cancelBtn) {
  cancelBtn.addEventListener('click', this._onClose.bind(this));
}
...
if (saveBtn) {
  saveBtn.addEventListener('click', this._onSave.bind(this));
}
...
if (cancelBtn) {
  ...
  private _onSave = async (): Promise<void> => {
    ...
    const errorDiv = this.domElement.querySelector(`.${styles.error}`);
if (errorDiv) {
  errorDiv.innerHTML = '';
}
    ...
    if (res.ok) {
      ...
      const errorDiv = this.domElement.querySelector(`.${styles.error}`);
if (errorDiv) {
  errorDiv.innerHTML = `An error has occurred while saving the item. Please try again. Error: ${error.error.message}`;
}
